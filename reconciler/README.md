...
| `loop next [--json]` | pick ([§8](../LOOP.md#8-operating-the-loop)) | Prints up to `policy.parallel` picks, grouping `batchable` same-stage items into one pick. Skips parked, cancelled, and `awaiting-approval` items. |
| `loop gate <item> <stage>` | gate + persist ([§3–4](../LOOP.md#3-gates)) | Runs the stage's gate; on pass commits the produced artifacts with the `Gate:`/`Inputs:`/`Produces:` trailers (empty commit for gate-only stages). On fail, prints the gate output and the note commands to classify it. |
| `loop note <op> <item>` | persist exceptions ([§5, §7](../LOOP.md#5-failure-taxonomy)) | Appends one well-formed line to `loop.notes.jsonl`. `attempt` auto-computes `n` and auto-parks as `needs-human` at `policy.maxAttempts`; `park` requires `--state blocked\|needs-human`. |
| `loop validate` | construction gate ([§8](../LOOP.md#8-operating-the-loop)) | Plan checks only (version, unique ids/stages, gate shape, `onFail` targets). |
| `loop run [--max-iters N]` | the pump | Fresh agent session per pick via `reconciler.json`; halts on parked items (configurable), on two no-progress sessions, or at `maxIters`; exits 0 when idle. |
...
- A stage's hashed inputs are the *per-item* artifacts of earlier stages; shared
  artifacts a stage also edits (e.g. common page objects) ride along in `produces` and
